# Fluxora Landing Page: The "Straight Talk" Edition

## 🚀 The Hero
- **Headline:** Stop guessing your AI bill. Audit it in 60 seconds.
- **Subheadline:** Most startups are double-paying for Cursor, Claude, and ChatGPT. We find the waste and tell you exactly how to fix it.
- **CTA:** [ Run My Free Audit ]

## 🤝 Social Proof (Mocked for now)
- "Fluxora found $300/mo in waste on my 5-person team. That's a few extra server instances for free." — *Founder of some AI Startup*
- "Finally, a tool that doesn't ask for my bank login just to tell me I'm overspending on seats." — *Random CTO on X*

## ❓ The "Common Questions" (FAQ)

**1. Is this just an AI wrapper?**
Nope. The math is 100% deterministic code. We only use AI to write the summary letter at the end so it sounds like a human wrote it. The "Brain" is pure financial logic.

**2. Is it safe?**
Yeah. We don't ask for your bank account or Stripe login. You just tell us what you're using, and we compare it against our pricing database. You keep your data, we just give you the report.

**3. What do I get at the end?**
A link you can send to your CFO that makes you look like a genius. Plus a PDF report and a detailed optimization roadmap to consolidate your active license contracts.

**4. How do you actually find savings?**
We look for "Redundancy." If you're paying for Cursor (which has Claude built-in) and also paying for Claude Pro for the same dev... that's a waste. We flag that stuff immediately.

**5. Is it really free?**
The audit is 100% free. We make money if you decide to book a specialized consultation or contract negotiation strategy with us later. No catch.
